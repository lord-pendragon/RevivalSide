param(
  ...
  [string[]]$RuntimeIdentifiers = @("win-x64", "win-arm64", "win-x86"),
...
if (-not $RuntimeCacheDir) {
  $RuntimeCacheDir = Join-Path $rootPath "prebuilt\official-profile-capture-runtimes"
}
...
function Get-NodeDownloadArchitecture([string]$Rid) {
  switch ($Rid) {
    "win-x64" { return "x64" }
    "win-x86" { return "x86" }
    "win-arm64" { return "arm64" }
...
function Resolve-NodeRuntime([string]$Rid) {
  ...
  $nodeArch = Get-NodeDownloadArchitecture $Rid
...
function Find-InstalledWiresharkDir([string]$ExpectedArchitecture) {
  ...
  if ($tsharkCommand) {
    $candidates += (Split-Path -Parent $tsharkCommand.Source)
  }
...
function Resolve-WiresharkRuntime([string]$Rid) {
  ...
  $process = Start-Process `
    ...
    -ArgumentList @("/S", "/desktopicon=no", "/D=$cachedDir") `
    -Wait `
    -PassThru `
    -WindowStyle Hidden
  if ($process.ExitCode -ne 0) {
    throw "Wireshark installer failed for $Rid with exit code $($process.ExitCode)"
  }
...
$packageScript = Join-Path $rootPath "tools\package-official-profile-capture-app.ps1"
$built = @()
foreach ($rid in $RuntimeIdentifiers) {
  $outputDir = Join-Path $OutputRoot "official-profile-capture-app-$rid"
  ...
  if ($IncludeGameplayJsons) { $packageArgs += "-IncludeGameplayJsons" }
